...
data_path = "./LLMsGreenRec/dataset/green_dataset/processed/"
...
initial_prompt = (
    """
    Based on the user's session interactions, you need to answer the following subtasks step by step:
    1. Discover combinations of items within the session, where the number of combinations can be one or more, focusing on patterns that suggest sustainability preferences.
    2. Based on the items within each combination, infer the user's interactive intent within each combination, emphasizing aspects related to sustainable choices.
    3. Select the intent from the inferred ones that best represents the user's current preferences for sustainability.
    4. Based on the selected intent, please rerank the 20 items in the candidate set according to the possibility of potential user interactions that align with sustainable habits, and show me your ranking results with item index.
